...
======================================================================
  ...
  运行方式（单次执行 / 定时任务直接调用这个脚本）：
    python daily_mailer.py
    #     → 默认读取同目录 config_daily_report.json
    python daily_mailer.py --config ./my.json
    #     → 指定配置文件路径
    python daily_mailer.py --dry-run
    #     → 只生成 HTML 报表，不发邮件（调试模式）
    python daily_mailer.py --test-smtp
    #     → 只测试 SMTP 连通性（发送空邮件）
    python daily_mailer.py --hours 24 --out ./xx.html
    #     → 临时覆盖参数（指定统计时段 / 输出路径）
======================================================================
"""
...
# =========================================================
# Mock ES 数据（本地无真实 ES 时，也能走完报表生成+邮件发送全流程）
# 返回结构完全匹配 elk_p99_report 模块 extract() / build_report() 要求
# =========================================================
def _mock_es_data(hours: int, bucket: str,
                  *, seed: int = 42) -> Dict[str, Any]:
    """返回 (code, result) 元组，200 + 符合ES _search 响应结构的假聚合"""
    import random as _r
    _r.seed(seed)
    import math as _m
    # --- 每分钟分桶数：按 bucket 粗略估算 ---
    bucket_minutes = {'1m': 1, '5m': 5, '10m': 10, '15m': 15, '30m': 30, '1h': 60}.get(bucket, 5)
    n_buckets = max(1, int(hours * 60 / bucket_minutes))
    now_ms = int(dt.datetime.now().timestamp() * 1000)
    step_ms = bucket_minutes * 60 * 1000
    start_ms = now_ms - step_ms * n_buckets

    def _base_sin_wave(i: int) -> float:
        """模拟真实业务的早高峰/晚高峰波形 + 随机抖动"""
        # 24小时1440分钟，每15分钟一个周期点，叠加昼夜波形
        phase = (i % 96) / 96 * 2 * _m.pi  # 96 buckets/day
        # 早高峰(10点) + 晚高峰(20点)两个凸起
        wave = 30 * (_m.sin(phase) + 0.8 * _m.sin(phase * 2 + 0.6) + 0.5 * _m.cos(phase * 3))
        return float(wave)

    per_min_buckets: List[Dict[str, Any]] = []
    for i in range(n_buckets):
        ms = start_ms + i * step_ms
        ts_str = dt.datetime.fromtimestamp(ms / 1000).strftime('%Y-%m-%dT%H:%M:%S.000+08:00')
        w = _base_sin_wave(i)
        p50 = round(max(20.0, 120.0 + w * 0.8 + _r.uniform(-8, 8)), 2)
        p95 = round(max(60.0, 450.0 + w * 3.2 + _r.uniform(-30, 30)), 2)
        p99 = round(max(120.0, 980.0 + w * 7.0 + _r.uniform(-80, 80)), 2)
        cnt = max(10, int(320 + w * 8 + _r.randint(-40, 60)))
        per_min_buckets.append({
            "key_as_string": ts_str,
            "key": ms,
            "doc_count": cnt,
            "cnt": {"value": cnt},
            "p50": {"values": {"50.0": p50}},
    ...
    # --- 整体百分位 ---
    overall_p50 = round(sum(b['p50']['values']['50.0'] for b in per_min_buckets) / len(per_min_buckets), 2)
    overall_p90 = round(overall_p50 * 2.7 + 120 + _r.uniform(-30, 30), 2)
    overall_p95 = round(sum(b['p95']['values']['95.0'] for b in per_min_buckets) / len(per_min_buckets), 2)
    overall_p99 = round(sum(b['p99']['values']['99.0'] for b in per_min_buckets) / len(per_min_buckets), 2)

    # --- 按 module（DBF / SP / 其它）---
    by_module = [
        {"key": "DBF", "doc_count": int(9400 + _r.randint(-400, 500)),
         "p99": {"values": {"99.0": round(overall_p99 * 0.88 + _r.uniform(-50, 50), 2)}}},
        {"key": "SP",  "doc_count": int(4800 + _r.randint(-300, 300)),
         "p99": {"values": {"99.0": round(overall_p99 * 1.12 + _r.uniform(-40, 70), 2)}}},
        {"key": "CACHE", "doc_count": int(380 + _r.randint(-80, 80)),
         "p99": {"values": {"99.0": round(35 + _r.uniform(-8, 20), 2)}}},
    ]

    # --- 按 branch（Admin按钮兜底的 10 个典型业务分支）---
    BRANCHES = [
        ("预约管理/提交预约单", 0.9, 1.05),
        ("预约管理/退订接口", 1.05, 1.18),
        ("座位图/实时选座查询", 0.6, 0.75),
        ...
        ("用户中心/我的预约", 0.75, 0.9),
        ("订单/支付回调", 1.25, 1.55),
        ("报表/数据导出Excel", 1.8, 2.4),
        ("活动/剧目详情", 0.5, 0.65),
        ("短信/邮件通知", 0.85, 1.1),
        ("登录/会话校验", 0.3, 0.42),
    ]
    by_branch: List[Dict[str, Any]] = []
    for name, p50f, p99f in BRANCHES:
        cnt = int(600 + _r.randint(50, 1500))
        p99 = round(overall_p99 * p99f + _r.uniform(-60, 60), 2)
        by_branch.append({
            "key": name,
            "doc_count": cnt,
            "cnt": {"value": cnt},
    ...
    # --- 兜底 vs 真实绑定 ---
    total_cnt = sum(m['doc_count'] for m in by_module)
    bound_true_cnt = int(total_cnt * 0.78 + _r.randint(-150, 200))
    bound_false_cnt = total_cnt - bound_true_cnt
    by_bound = [
        {"key": True,  "doc_count": bound_true_cnt,  "cnt": {"value": bound_true_cnt}},
        {"key": False, "doc_count": bound_false_cnt, "cnt": {"value": bound_false_cnt}},
    ]

    result: Dict[str, Any] = {
        "took": 18 + _r.randint(3, 30),
        "timed_out": False,
        "_shards": {"total": 5, "successful": 5, "skipped": 0, "failed": 0},
        "hits": {
            "total": {"value": total_cnt, "relation": "eq"},
            "max_score": None,
            "hits": [],
        },
        "aggregations": {
            "overall_count": {"value": total_cnt},
            "overall_percentiles": {
                "values": {
                    "50.0": overall_p50,
                    "90.0": overall_p90,
            ...
            "per_minute": {"buckets": per_min_buckets},
            "by_module": {"buckets": by_module},
            "by_branch": {"buckets": by_branch},
            "by_bound": {"buckets": by_bound},
        },
    }
    return 200, result
...
def gen_report(cfg: Dict[str, Any], logger: Logger, args: argparse.Namespace) -> Tuple[Dict[str, Any], str, Dict[str, Any]]:
    ...
    # ============ --mock-es 分支：不碰真实ES，直接用内置假数据 ============
    if getattr(args, 'mock_es', False):
        logger.log('INFO', f'[ES_QUERY_BEGIN] MODE=MOCK_ES（--mock-es已启用，跳过真实ES查询） '
                          f'index={fake_cfg_ns.index} 最近{hours}h 分桶{bucket}')
        code, result = _mock_es_data(hours, bucket, seed=int(time.time()) % 100000)
        logger.log('INFO', f'[MOCK_ES_GENERATE] OK took_mock_ms={(result or {}).get("took", -1)}')
    else:
...
最慢 Top5 分支（按 P99 倒序）：
{chr(10).join(f'  {i+1}. {r["branch"]:<22s} P99={fv(r["p99"], " ms")}  点击{r["count"]:,} 次' for i, r in enumerate(by_branch_top))}
...
def main() -> int:
    ...
    # ==== 新增：Mock ES 数据（本地无真实ES时也能走完报表生成+邮件发送全流程）====
    p.add_argument('--mock-es', action='store_true',
                   help='使用内置假的 P99 聚合数据（不调用真实 ES），用于本地演示/无ES环境的端到端验证')
    # ==== 新增：Verbose 调试开关 ====
